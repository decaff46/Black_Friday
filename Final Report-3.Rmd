---
title: "Final Report_Black Friday"
author: "Brad Kim, Casey O’Malley,  Qi Ding, Caffrey Lee,  Zoe Huang"
date: ""
output:
...
1. We found many interesting facts about customer-behaviour during Black friday as follows;

2. Besides the analysis below, we also created a separate shiny reporting engine so that user can compare the result of each variable more easily.
...
Now that we have the data table necessary to see each User_IDs corresponding gender and their total counts for reference, let's plot the distribution of gender across our dataset and take a look at their purchasing record.
...
**Interesting fact 1**: As we can see, there are quite a few more males than females shopping at our store on Black Friday. But it could also mean less number of females paid for the products and may be their spouse paid for them. 
...
**Interesting fact 6**: Unexpectedly, Purchasing power is not directly related to the amount of purchases for each customer. It is clear that City A has the least number of customers; however, their purchasing power per customer is the highest. People from Area A and B have a greater purchasing power than others, and greater sales gained from people from Area C
...
**Interesting fact 7**: One inference we can make from these charts is that customers from City B are simply making more purchases than residence of City A + City C, and not necessarily buying more expensive products. It is safe to make that claim due to the fact that the City B has the most Purcahse_Count by far different from the others, but its purchasing power and Amount_per Transaction is similar to that of City A and even lower than City C.
...
**Interesting fact 9**: It seems that these top products are loved by people from city C. Also compared with the overall distribution pie chat, people from city C purchased the best selling product more than other shoppers.

###OCCUPATION

Next, let's examine if people doing different kinds of jobs would have different purchasing behaviour, focusing on if people from specific occupations would spend more.
...
It can be seen that the occupational category has a lot to do with the amount of consumption. This specific store can provide products that are more in line with professional characteristics according to the top ranked occupations.
...
**Interesting fact 10**: People who live for a year consume the most, and then decrement, which makes sense in real life. When you first come to the city, you need to buy all kinds of daily necessities.
...
We will provide insight to the black friday sales using clustering with k-means algorithm to inspect the characteristics of each cluster based on (1) needs-based variables, and (2) demographics. 
  ...
  - Profiling clusters by needs-based variables will help us understand how the market segments differ in their needs which in turn will aid the firm in offering products and services to matches the unique needs of the segment. 
...
In this case, since we do not have much information about products, we will only use demographics to indentify target customers.
...
```{r pre_cluster_analysis}
#Make the categorical dummy variable using dummyVars function in caret
dummy <- dummyVars("~ Gender + Age + Occupation + City_Category + Stay_In_Current_City_Years + Marital_Status", data = data)
new_df <- data.frame(predict(dummy, newdata = data))

#Merge the two datasets together
new_data <- cbind(data, new_df)

#Select variables that will be used in later clustering:
cluster_data <- new_data %>% select(-User_ID, -Product_ID, -Gender, -Age, -City_Category, -Occupation, -Stay_In_Current_City_Years, -Marital_Status, -Product_Category_1, -Product_Category_2, -Product_Category_3, -Purchase)
...
Unfortunately, this work did not yield great results, as the elbow plot generated by our model did not show any “elbow” where the slope of the line flattens out.  Instead, the graph presents a relatively straight shape for the first eight values and is difficult to interpret. Here, we will go with a four-cluster solution.

```{r kmeans_model_demographic_based}
#build a kmeans model
model_km4 <- kmeans(cluster_data, centers = 4)
#extract the cluster
clust_km4 <- model_km4$cluster
black_friday_km4 <- mutate(as.data.frame(cluster_data), cluster = clust_km4)
head(black_friday_km4)
#count the number of customer in each cluster:
count(black_friday_km4, cluster)
#mutate the result back into the original data set
blackfriday_demographic_based <- data %>% 
                  mutate(cluster = clust_km4)
...
When we take more variables into consideration, the results can be used to target customer segments and develop marketing campaign. Despite our efforts, none of these clusters had significantly different purchasing habits than the overall populations, which prohibited us from making any actionable insights based on the results that could increase sales.

2. Use k-means segments to examine means of each demographic-based variable. 

```{r kmeans_segments_demographic_based_Gender}
data2 <- cbind(data,clust_km4)
library(ggplot2)
library(RColorBrewer) 
tab1 = prop.table(table(data2$clust_km4,data2[,3]),1)
...
tab2 = prop.table(table(data2$clust_km4,data2[,4]),1)
...
ggplot(data=tab2,aes(x=Var2,y=Var1,fill=Freq))+
   ...
   ylab(label = '')+
   scale_fill_gradientn(colors=brewer.pal(n=9,name = 'Greens'))
tab3 = prop.table(table(data2$clust_km4,data2[,6]),1)
...
Based on these results, we can find that most of people in the first cluster are males and aging from 26 to 35. With these information, we can target our customer to increase the sale. 

Normally, profiling clusters on demographics or observable variables will help identify customers and target them with the right offer. But in this case, kmeans result is really disappointing.

#Part4: Recommender system
